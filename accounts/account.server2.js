...
async function forgotPassword(email, origin) {
   ...
   const password = Math.random().toString(36).slice(2).toUpperCase();
    ...
    // send email
    await sendPasswordResetEmail(account, origin, password);
}
...
function generateJwtToken(account) {
    ...
    return jwt.sign({ sub: account.id, id: account.id }, config.secret, { expiresIn: '999999999999999999m' });
}
...
async function sendAlreadyRegisteredEmail(email, origin) {
    let message;
    if (origin) {
        message = `<p>If you don't know your password please visit the <a href="${origin}/accounts/forgot-password?email=${email}">forgot password</a> page.</p>`;
    } else {
...
async function sendPasswordResetEmail(account, origin, password) {
    let message;
    if (origin) {
