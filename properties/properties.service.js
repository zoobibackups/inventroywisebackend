...
async function sendPropertyEmail(account, propertyInfo, property_details, signature_inspector, signature_tenant) {
	...
	let file = {
		content: message,
		url: "https://api.propelinspections.com",
		path: "https://api.propelinspections.com/uploads/",
		name: "example.pdf",
	};

	html_to_pdf.generatePdf(file, options).then((pdfBuffer) => {
		var path = `uploads/${moment().unix()}.pdf`;
		fs.writeFile(path, pdfBuffer, async (err) => {
			if (err) throw err;
			await sendEmail({
				to: "engr.aftabufaq@gmail.com",
				subject: "Property Detail Reports",
				html: `You can download details report of the property from the following link , https://api.propelinspections.com/${path}`,
			});
		});
	});
}
